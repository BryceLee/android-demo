...
/**
 * Created by lizhongxin on 30/10/2018.
 */
@RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN_MR2)
public class VideoKeyFrame {
    private final static String TAG = "VideoKeyFrame";
    ...
    public boolean VideoKeyFrame(final Context context, String url, long clipPoint, long clipDuration) {
        ...
        try {
            ...
            //创建合成器
            File file = new File(url.substring(0, url.lastIndexOf(".")) + "_VideoKeyFrame.mp4");
...
//            inputBuffer.clear();
            videoInfo.presentationTimeUs = presentationTimeUs;//presentationTimeUs;
...
//            audioInfo.presentationTimeUs += audioSampleTime;//presentationTimeUs;
        ...
        if (context!=null){
            ((Activity)context).runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(context,"done,耗时："+(System.currentTimeMillis()-start),Toast.LENGTH_SHORT).show();
                }
            });
        }
