...
/**
 * Created by lizhongxin on 03/08/2018.
 */

public class CheckExitService extends Service {

  private String packageName = "textjunit.lizhongxin.demo";
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    ...
    List<ActivityManager.RunningAppProcessInfo> runningAppProcesses = activtyManager
        .getRunningAppProcesses();
    for (int i = 0; i < runningAppProcesses.size(); i++) {
      if (packageName.equals(runningAppProcesses.get(i).processName)) {
        Log.d("packageNameBQ:", "" + runningAppProcesses.get(i).processName);
